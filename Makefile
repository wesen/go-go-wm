...
logcopter-check:
	GOWORK=off go tool logcopter-gen -area-prefix go-go-golems.go-go-wm -strip-prefix github.com/go-go-golems/go-go-wm -check ./pkg/...
...
release:
	...
	GOWORK=off GOPROXY=proxy.golang.org go list -m github.com/go-go-golems/go-go-wm@$(shell svu current)
...
GO_GO_WM_BINARY=$(shell which go-go-wm)
install:
	GOWORK=off go build -o ./dist/go-go-wm ./cmd/go-go-wm && \
		cp ./dist/go-go-wm $(GO_GO_WM_BINARY)
